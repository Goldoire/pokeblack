#ifndef POKEBLACK_G_CLACT_H
#define POKEBLACK_G_CLACT_H
/*
 * ===========================================================================
 *  clact.c -- the cell-actor / 2D sprite system (main 0x0204A48C-0x0204E074)
 * ===========================================================================
 *
 * PROMOTED from src/main/g_clact.h. Delete that file and include this.
 *
 * Every offset marked PROVEN is constrained by a byte-exact function in
 * src/main/g_clact.c, named in the comment. Naming follows pokeheartgold's
 * sprite.h where the gen-4 function corresponds.
 *
 * The bitfield word at GClact+0x60 is the strongest single piece of evidence
 * in this header: every boundary is the bic mask of a verified accessor, and
 * the twelve fields sum to exactly 32 bits with one 1-bit gap (unk60_18)
 * inferred to close it. A partition that adds up is very hard to get wrong by
 * accident.
 *
 * NOTE on the GNNS* types: these are layout-identical local copies of the NNS
 * structures, kept because sub_0204CAE4 initialises actor+0x1C with
 * NNSi_G2dInitializeVRamLocation and actor+0x40 with
 * NNS_G2dInitImagePaletteProxy. The real NNSG2dImageProxy /
 * NNSG2dImagePaletteProxy are available from <nnsys.h> via global.h and
 * should replace these once someone confirms the sizes match (0x24 / 0x14).
 */
