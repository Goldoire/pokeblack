 ...
 *
 * The list node ("Node") carries a 0x8000-byte receive buffer at +0x40 with
 * its cursor/count at +0x8040/+0x8044 and the refcount sub_021BEFE4 bumps at
 * +0x8048.  The Sock hanging off node->unk_10 owns the SSL block (+0xAC).
...
extern int sub_0215F480(int fd, const SockAddr *sa);
...
extern HostEnt *sub_0215F650(const char *name);
extern int sub_0215FAD0(const char *name, u32 *out, int flags);
extern int sub_0215FC80(int fd, Ssl *ssl);
...
extern Node *sub_021BEBE0(Mgr *m, void *key);

int sub_021B9E3C(int a, int b, Sock *s, int fd);
int sub_021B9ECC(Node *n, int fd, void *dst, int len, int flags);
...
int sub_021B9D8C(int a, int b, Sock *s, int fd, u32 ip, u32 port)
{
    SockAddr sa;
    u16 p = (u16)port;
    int rc;
...
int sub_021B9F94(Mgr *m, Sock *s, int fd, void *dst, int len, int flags)
{
    Node *n = sub_021BEBE0(m, s);
    int rc;
...
int sub_021BA020(Sock *s, int fd, const void *buf, int len, int flags)
{
